...
- CSS decodes the full real x86 `B0..B7` `MOV r8,ib` and `B8..BF` `MOV r16,iw` families; the working word-width spine includes `MOV` (`89/8B`), `XOR` (`31/33`), register `ADD/SUB r16,r/m16` (`03/2B`), register `XCHG r/m16,r16` (`87`), register `SHL r/m16,1` (`D1 /4`), `CMP AX,iw` (`3D`), effective-address `LEA r16,m` (`8D`), register and memory-source `MOV Sreg,r/m16` (`8E`, valid `ES/SS/DS` only), memory `LDS` (`C5`), byte string loads `LODSB` (`AC`), `CBW` sign extension (`98`), immediate ALU, stack, near/far branch/call/return, string, and control instructions;
