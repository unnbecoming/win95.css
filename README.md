...
- CSS decodes the full real x86 `B8..BF` `MOV r16,iw` family, `MOV [moffs16],AX`, `JMP rel16`, `CALL rel16`, `RET`, `ADD AX,iw`, `SUB AX,iw`, `XOR AX,iw`, and `HLT` opcodes;
...
The generated ALU is **376 registered one-bit nets / 59,974 CSS bytes**. The stack-capable CPU is **940 nets / 200,998 CSS bytes** and exposes `AL/CL/DL/BL/AH/CH/DH/BH` aliases in its manifest.
