...
- the byte-width spine includes register and memory `MOV r/m8,r8` (`88`), `MOV r8,r/m8` (`8A`), `XOR r8,r/m8` (`32`), register `OR r8,r/m8` (`0A`), `CMP r/m8,r8` (`38`), `MOV r/m8,ib` (`C6 /0`), register `SUB/CMP r/m8,ib` (`80 /5,/7`), register `SHL r/m8,1` (`D0 /4`), register `ROL r/m8,ib` (`C0 /0`), `MOV AL,moffs8` (`A0`), and `AND/OR AL,ib` (`24/0C`);
...
The generated ALU is **376 registered one-bit nets / 59,974 CSS bytes**. The CPU-plus-current-device slice is **3,340 nets / 1,555,078 CSS bytes** and exposes `AL/CL/DL/BL/AH/CH/DH/BH` aliases plus manifest-declared memory, port-output, DOR, reset, controller-INT, and IRQ6-request contracts.
