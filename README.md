...
- real-mode default-segment selection, segment-register write decode, two-byte memory and stack read/write/RMW microphases, four-byte far-pointer reads, `MOVSB` source/write cycles, CSS-owned `SI/DI/CX` string updates, architectural SP ordering, `IF`/`DF` update, and 20-bit physical address formation;
