...
const opcodes = { addRm8Reg: 0x00, add: 0x05, addRegRm: 0x03, orRegRm8: 0x0a, orAlImm8: 0x0c, andAlImm8: 0x24, xor: 0x35, csOverride: 0x2e, subRegRm8: 0x2a, subRegRm: 0x2b, xorRmReg: 0x31, xorRegRm8: 0x32, xorRegRm: 0x33, sub: 0x2d, cmpAlImm8: 0x3c, cmpRm8Reg: 0x38, groupRm8Imm: 0x80, testRm8Reg: 0x84, jb: 0x72, jbe: 0x76, xchgRmReg: 0x87, movRm8Reg: 0x88, movRegRm8: 0x8a, movRmReg: 0x89, movRegRm: 0x8b, lea: 0x8d, movSreg: 0x8e, movAlMoffs8: 0xa0, store: 0xa3, movsb: 0xa4, lodsb: 0xac, lds: 0xc5, movRm8Imm: 0xc6, rolRm8Imm: 0xc0, shlRm8One: 0xd0, shlRm16One: 0xd1, lock: 0xf0, rep: 0xf3, jz: 0x74, jnz: 0x75, jl: 0x7c, retfImm: 0xca, intImm: 0xcd, iret: 0xcf, loop: 0xe2, call: 0xe8, testRm8Imm: 0xf6, decRm8: 0xfe, callRm16: 0xff, inDxAl: 0xec, outDxAl: 0xee, jmpShort: 0xeb, jmp: 0xe9, far: 0xea, ret: 0xc3, cbw: 0x98, pushf: 0x9c, popf: 0x9d, cmc: 0xf5, clc: 0xf8, stc: 0xf9, cli: 0xfa, sti: 0xfb, cld: 0xfc, hlt: 0xf4 };
...
signal['word-memory-write'] = andBit(andBit(andBit(andBit(notBit(ref('opcode-lds')), notBit(ref('opcode-movsb'))), notBit(ref('opcode-lodsb'))), notBit(ref('opcode-byte-memory-write'))), ref('memory-final-pair'));
...
signal['read-phase'] = anyBits([ref('instruction-read'), ref('stack-read'), ref('memory-read'), ref('lds-extra-read'), ref('movsb-read'), ref('lodsb-read'), ref('int-vector-read')]);
...
signal['fetched-immediate'] = anyBits([...['mov', 'add', 'sub', 'cmpAlImm8', 'xor', 'andAlImm8', 'orAlImm8', 'movAlMoffs8', 'store', 'call', 'jmp', 'far', 'retfImm'].map((name) => ref(`fetched-${name}`)), ref('fetched-mov8')]);
...
signal['fetched-simple'] = anyBits([ref('fetched-if-control'), ref('fetched-cbw'), ref('fetched-cmc'), ref('fetched-clc'), ref('fetched-stc'), ref('fetched-cld')]);
...
signal['fetched-string'] = orBit(ref('fetched-movsb'), ref('fetched-lodsb'));
...
signal['cs-override-target-invalid'] = andBit(ref('csOverride'), notBit(anyBits([ref('fetched-prefix'), ref('fetched-modrm'), ref('fetched-lodsb')])));
...
signal['opcode-memory-supported'] = anyBits([ref('opcode-lea'), ref('opcode-addRm8Reg'), ref('opcode-movRm8Reg'), ref('opcode-movRegRm8'), ref('opcode-movRmReg'), ref('opcode-movRegRm'), ref('opcode-xorRmReg'), ref('opcode-xorRegRm8'), ref('opcode-xorRegRm'), ref('opcode-cmpRm8Reg'), ref('opcode-testRm8Reg'), ref('opcode-movSreg'), ref('opcode-lds'), ref('opcode-movRm8Imm'), ref('opcode-callRm16'), ref('opcode-testRm8Imm'), ref('opcode-groupRm8Imm')]);
signal['opcode-memory-to-reg'] = orBit(ref('opcode-movRegRm'), ref('opcode-xorRegRm'));
signal['opcode-memory-needs-read'] = anyBits([ref('opcode-memory-to-reg'), ref('opcode-addRm8Reg'), ref('opcode-xorRmReg'), ref('opcode-xorRegRm8'), ref('opcode-cmpRm8Reg'), ref('opcode-testRm8Reg'), ref('opcode-movRegRm8'), ref('opcode-movSreg'), ref('opcode-lds'), ref('opcode-callRm16')]);
...
signal['shl-rm16-one-selector-invalid'] = andBit(ref('capture-modrm'), andBit(ref('opcode-shlRm16One'), notBit(ref('shl-rm16-one-selector-valid'))));
...
signal['modrm-invalid'] = anyBits([ref('modrm-address-invalid'), ref('mov-sreg-selector-invalid'), ref('lds-register-invalid'), ref('lea-register-invalid'), ref('lock-register-invalid'), ref('mov-rm8-selector-invalid'), ref('group-rm8-immediate-selector-invalid'), ref('shl-rm8-one-selector-invalid'), ref('shl-rm16-one-selector-invalid'), ref('call-rm16-selector-invalid'), ref('call-rm16-register-invalid'), ref('rol-rm8-immediate-selector-invalid'), ref('dec-rm8-selector-invalid'), ref('test-rm8-immediate-selector-invalid'), ref('test-rm8-immediate-register-invalid')]);
...
signal['clear-cs-override'] = anyBits([
  ref('modrm-commit'),
  andBit(ref('capture-memory-low'), anyBits([ref('opcode-cmpRm8Reg'), ref('opcode-testRm8Reg'), ref('opcode-movRegRm8'), ref('opcode-xorRegRm8')])),
...
signal['continue-immediate'] = andBit(ref('capture-imm-low'), andBit(notBit(anyBits([ref('opcode-short'), ref('opcode-mov8'), ref('opcode-movRm8Imm'), ref('opcode-groupRm8Imm'), ref('opcode-al-logical-immediate'), ref('opcode-cmpAlImm8'), ref('opcode-rolRm8Imm')])), notBit(ref('opcode-intImm'))));
...
const phaseRoutes = [
  [1, anyBits([ref('begin-immediate'), ref('begin-io'), ref('begin-mov-rm8-register'), ref('begin-sub-rm8-immediate-register'), ref('begin-cmp-rm8-immediate-register'), ref('begin-rol-rm8-immediate-register')])], [2, ref('continue-immediate')], [3, orBit(ref('begin-write'), ref('begin-int'))], [4, ref('phase-write-low')],
  ...
  [12, anyBits([ref('memory-start-read'), ref('mov-rm8-immediate-start'), ref('test-rm8-immediate-start'), ref('group-rm8-immediate-start'), ref('begin-mov-al-moffs8'), andBit(ref('opcode-intImm'), ref('phase-disp-high')), andBit(ref('opcode-iret'), ref('phase-far-high'))])], [13, anyBits([ref('test-rm8-operand-start'), ref('group-rm8-operand-start'), ref('capture-iret-flags-low'), andBit(ref('capture-memory-low'), notBit(anyBits([ref('opcode-addRm8Reg'), ref('opcode-cmpRm8Reg'), ref('opcode-testRm8Reg'), ref('opcode-movRegRm8'), ref('opcode-xorRegRm8'), ref('opcode-movAlMoffs8')])) )])],
  [14, anyBits([ref('memory-start-write'), ref('memory-rmw-start-write'), ref('byte-rmw-start-write'), ref('lds-continue-read'), ref('begin-movsb'), ref('begin-lodsb'), ref('repeat-movsb'), ref('mov-rm8-write-start')])], [15, andBit(ref('phase-memory-write-low'), notBit(anyBits([ref('opcode-byte-memory-write'), ref('opcode-lodsb')])) )],
];
...
for (let index = 0; index < WIDTH; index++) {
  ...
  signal[`bus-offset-${index}`] = anyBits([
    ...
    andBit(anyBits([ref('movsb-read'), ref('lodsb-read')]), ref(`si-${index}`)),
  ...
  signal[`bus-segment-${index}`] = anyBits([
    ...
    andBit(ref('lodsb-read'), muxBit(ref('csOverride'), ref(`ds-${index}`), ref(`cs-${index}`))),
...
for (let index = 0; index < 8; index++) {
  ...
  signal[`modrm-byte-subtract-destination-${index}`] = muxBit(ref('sub-reg-rm8-register-commit'), ref(`modrm-rm-byte-register-value-${index}`), ref(`modrm-byte-register-value-${index}`));
  signal[`modrm-byte-subtract-source-${index}`] = muxBit(ref('sub-reg-rm8-register-commit'), ref(`modrm-byte-register-value-${index}`), ref(`modrm-rm-byte-register-value-${index}`));
...
for (const register of registers) {
  ...
  signal[`write-shl-rm16-${register}`] = andBit(ref('execute-shl-rm16-one-register'), ref(`modrm-rm-${register}`));
  ...
  signal[`write-${register}`] = anyBits([ref(`write-immediate-${register}`), ref(`write-modrm-${register}`), ref(`write-memory-${register}`), ref(`write-lea-${register}`), ref(`write-pop-${register}`), ref(`write-shl-rm16-${register}`), ref(`write-xchg-${register}`)]);
  for (let index = 0; index < WIDTH; index++) {
    ...
    signal[`write-source-${register}-${index}`] = muxBit(ref(`write-shl-rm16-${register}`), popSource, ref(`shl-rm16-result-${index}`));
    ...
    if (byteRegister) {
      ...
      const writeModrmByte = anyBits([writeImmediateModrmByte, writeAddRm8Reg, writeSubRm8Immediate, writeSubRegRm8, writeRegisterModrmByte, writeRegisterFromRmByte, writeMemoryToRegisterByte, writeByteLogicalRm8, writeDecRm8, writeShlRm8, writeRolRm8]);
      ...
      const addSource = muxBit(writeAddRm8Reg, decrementSource, ref(`add-rm8-reg-result-${index % 8}`));
      ...
      const cbwSource = muxBit(writeCbw, lodsbSource, ref('ax-7'));
      const inputSource = muxBit(writeInDxAl, cbwSource, ref(`busData-${index % 8}`));
      const writeByte = anyBits([writeImmediateByte, writeModrmByte, writeAlLogical, writeAlMoffs, writeLodsb, writeCbw, writeInDxAl]);
      signal[`next-${register}-${index}`] = muxBit(writeByte, ref(`next-base-${register}-${index}`), inputSource);
    } else if (register !== 'sp') {
...
for (let index = 0; index < WIDTH; index++) {
  ...
  signal[`next-si-${index}`] = muxBit(orBit(ref('finish-movsb'), ref('finish-lodsb')), nextSi, steppedSi);
...
for (const flag of ['cf', 'pf', 'af', 'zf', 'sf', 'of']) {
  ...
  const shiftedByteFlag = muxBit(ref('execute-shl-rm8-one-register'), logicalFlag, ref(`shl-rm8-${flag}`));
  const shiftedFlag = muxBit(ref('execute-shl-rm16-one-register'), shiftedByteFlag, ref(`shl-rm16-${flag}`));
  ...
  const testRm8RegFlag = muxBit(ref('test-rm8-reg-commit'), decFlag, ref(`test-rm8-reg-${flag}`));
  ...
  signal[`selected-${flag}`] = muxBit(ref('cmp-commit'), addRm8RegFlag, ref(`cmp-${flag}`));
