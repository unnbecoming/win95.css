...
signal['word-memory-write'] = andBit(andBit(andBit(andBit(andBit(andBit(notBit(ref('opcode-lds')), notBit(ref('opcode-movsb'))), notBit(ref('opcode-stosw'))), notBit(ref('opcode-lodsb'))), notBit(ref('opcode-lodsw'))), notBit(ref('opcode-byte-memory-write'))), ref('memory-final-pair'));
...
signal['write-phase'] = anyBits([ref('phase-write-low'), ref('phase-write-high'), ref('memory-write'), ref('movsb-write'), ref('stosw-write'), ref('int-stack-write')]);
...
signal['fetched-string'] = anyBits([ref('fetched-movsb'), ref('fetched-stosw'), ref('fetched-lodsb'), ref('fetched-lodsw')]);
...
signal['rep-target-invalid'] = andBit(ref('rep'), notBit(anyBits([ref('fetched-rep'), ref('fetched-csOverride'), ref('fetched-movsb'), ref('fetched-stosw')])));
...
signal['clear-rep'] = anyBits([ref('skip-rep-movsb'), ref('skip-rep-stosw'), andBit(ref('finish-movsb'), notBit(ref('repeat-movsb'))), andBit(ref('finish-stosw'), notBit(ref('repeat-stosw')))]);
...
const phaseRoutes = [
  ...
  [14, anyBits([ref('memory-start-write'), ref('memory-rmw-start-write'), ref('byte-rmw-start-write'), ref('lds-continue-read'), ref('begin-movsb'), ref('begin-stosw'), ref('begin-lodsb'), ref('begin-lodsw'), ref('repeat-movsb'), ref('repeat-stosw'), ref('mov-rm8-write-start')])], [15, andBit(ref('phase-memory-write-low'), notBit(anyBits([ref('opcode-byte-memory-write'), ref('opcode-lodsb')])) )],
];
...
for (let index = 0; index < 8; index++) {
  ...
  const stringBusWriteData = muxBit(ref('stosw-write'), ordinaryBusWriteData, stoswWriteData);
...
for (let index = 0; index < WIDTH; index++) {
  ...
  const movedCx = muxBit(orBit(ref('finish-movsb'), ref('finish-stosw')), nextCx, repeatedCx);
  signal[`next-si-${index}`] = muxBit(anyBits([ref('finish-movsb'), ref('finish-lodsb'), ref('finish-lodsw')]), nextSi, selectedSiStep);
  signal[`next-di-${index}`] = muxBit(orBit(ref('finish-movsb'), ref('finish-stosw')), nextDi, selectedDiStep);
